...
class Node(metaclass=ABCMeta):
    ...
    def __hash__(self):
        print("hash called for ", self.id)
        return self.id

    def __eq__(self, other):
        if isinstance(other, self.__class__):
            print("eq called for ", self.id, " and ", other.id)
        ...
        for node_batch in self.__iter_nodes__():
            ...
            for gate in node_batch:
                ...
                if isinstance(gate, Inverter):
                    mem[id] = neg(mem[gate.input.id])
                elif isinstance(gate, AndGate):
                    mem[id] = and_(mem[gate.left.id], mem[gate.right.id])
                elif isinstance(gate, Input):
                    mem[id] = lift(inputs[gate.name])
                elif isinstance(gate, LatchIn):
                    mem[id] = lift(latchins[gate.name])
                elif isinstance(gate, ConstFalse):
                    mem[id] = lift(False)

                if (id in latch_keys) or (id in node_keys):
                    store[id] = mem[id]  # Store for eventual output.
